...
public:
	...
	UFUNCTION(BlueprintCallable, Category = WebSocket)
	void SendLoginMsg(const FString& MatchName,const FString& MatchChildID,const TArray<FString>& Bikes );

	UFUNCTION(BlueprintCallable, Category = WebSocket)
	void SendMatchMsg(const FString& matchName,const FString& openid, const FString& bikename, const FString & RankIndex, const FString& kll, const FString& time, const FString& Avgspeed);

	UFUNCTION(BlueprintCallable, Category = WebSocket)
	void SendMatchOverMsg(const FString& MatchName, const FString& MatchChildID);

	UFUNCTION(BlueprintCallable, Category = WebSocket)
	void SendUserExitMsg(const FString& openid,const FString& MachineName);
